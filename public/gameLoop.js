import { entities, player, difficulty, tileMap2D, allmaps, mapDisplayName, loadNextLevel } from "./bomber.js";
...
let keysCollected = 0; // total keys the run
...
export function gameLoop(time) {
    ...
    entities.forEach(e => {
        ...
        if (e instanceof Objective) {
            // if it’s the key
            if (!e.collected && e.el.classList.contains("key") && collision(player.bounds, e.bounds)) {
                e.el.remove();
                e.collected = true;
                ...
                if (!midStoryShown) {
                    midStoryShown = true;
                    showMidStory();  // showMidStory itself checks allmaps
                }

                // Speed up all enemies (both modes)
                entities.forEach(en => {
            ...
            if (!levelCompleted && e.el.classList.contains("port") && collision(player.bounds, e.bounds)) {
                if (player.hasKey) {
                    //  Story mode (allmaps = true): go through all maps
                    if (allmaps && keysCollected < 4) {
                        portSpawned = false;
                        levelCompleted = false;
                        player.hasKey = false;
                        loadNextLevel();   // swap tileMap, rebuild entities, etc.
                        return;            // skip end-of-game stuff for intermediate levels
                    }

                    //  Single-map mode OR last map in story -> finish run
                    levelCompleted = true;
                    stopMusic();
...
function playerEat(powerUp) {
    ...
    if (powerUp.type === "flake") {
        freezeEnemiesFor7Seconds();
        PlayPowerUpSound();
        addScore(20);
        powerUp.el.remove();
...
function freezeEnemiesFor7Seconds() {

    if (enemiesFrozen) return;

    enemiesFrozen = true;

    entities.forEach(enemy => enemy.frozen = true);

    setTimeout(() => {
        enemiesFrozen = false;
        entities.forEach(enemy => enemy.frozen = false);
    }, 7000);
}